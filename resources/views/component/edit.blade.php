...
@section('content')
    <div class="container mx-auto p-6">
        <div class="overflow-x-auto">
            <h2 class="text-2xl font-bold text-center mb-6">Edit Component</h2>
            ...
            <form action="{{ route('component.update', $component->id) }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="brand_id" class="select">
                        @foreach($brands as $brand)
                            <option value="{{ $brand->id }}" {{ $brand->id == $component->brand_id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" step="0.01" name="original_price" class="input input-bordered w-full" required value="{{$component -> original_price}}">
                </div>
                <div>
                    <label class="">Discount (%)</label>
                    <input type="text"  name="discount" class="input input-bordered w-full"  value="{{$component -> discount}}">
                </div>

                <div>
                    <label class="">Promotional price (VND)</label>
                    <input type="text" step="0.01" name="promotional_price" class="input input-bordered w-full"  value="{{$component -> promotional_price}}">
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" class="textarea textarea-bordered w-full" required>{{ $component -> description }}</textarea>
                </div>
