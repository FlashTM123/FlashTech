                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm text-center">
                                            <div class="flex justify-center space-x-2">
                                                <a href="{{ route('employees.edit', $employee->id) }}" class="btn btn-outline btn-warning">
                                                    Edit
                                                </a>

                                                <form action="{{ route('employees.destroy', $employee->id) }}" id="delete-form-{{ $employee->id }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-outline btn-error" onclick="confirmDelete({{ $employee->id }})">
                                                        Delete
                                                    </button>
                                                </form>
                                            </div>
